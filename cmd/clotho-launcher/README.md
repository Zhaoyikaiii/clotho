...
A standalone launcher for Claw, providing visual JSON editing and OAuth provider authentication management.
...
```bash
# Build
go build -o clotho-launcher ./cmd/clotho-launcher/

# Run with default config path (~/.clotho/config.json)
./clotho-launcher

# Specify a config file
./clotho-launcher ./config.json

# Allow LAN access
./clotho-launcher -public
```
...
```
Usage: clotho-config [options] [config.json]

Arguments:
  config.json    Path to the configuration file (default: ~/.clotho/config.json)
...
{
  "config": { ... },
  "path": "/Users/xiao/.clotho/config.json"
}
```
...
After authentication, Google redirects to `GET /auth/callback`, which saves the credentials and redirects back to the clotho-config UI.
...
Gets the running status of the `clotho gateway` process.
...
Starts the `clotho gateway` process in the background.
...
Stops the running `clotho gateway` process.
