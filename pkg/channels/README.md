# Claw Channel System: Complete Development Guide

> **Scope**: `pkg/channels/`, `pkg/bus/`, `pkg/media/`, `pkg/identity/`, `cmd/clotho/internal/gateway/`
...
import (
    "github.com/Zhaoyikaiii/clotho/pkg/bus"
    "github.com/Zhaoyikaiii/clotho/pkg/channels"     // Reference parent package
    "github.com/Zhaoyikaiii/clotho/pkg/config"
    "github.com/Zhaoyikaiii/clotho/pkg/identity"      // New
    "github.com/Zhaoyikaiii/clotho/pkg/media"          // New (if media support needed)
)
```
...
```go
// cmd/clotho/internal/gateway/helpers.go
import (
    _ "github.com/Zhaoyikaiii/clotho/pkg/channels/telegram"   // Triggers init() registration
    _ "github.com/Zhaoyikaiii/clotho/pkg/channels/discord"
    _ "github.com/Zhaoyikaiii/clotho/pkg/channels/your_new_channel"  // New addition
)
```
...
// cmd/clotho/internal/gateway/helpers.go
import (
    _ "github.com/Zhaoyikaiii/clotho/pkg/channels/matrix"
)
```
...
| `pkg/channels//` | `""` | TypingCapable, PlaceholderCapable, MessageEditor, WebhookHandler |
...
4. ** Protocol**: `pkg/channels//` implements a custom Claw native protocol channel that receives messages via WebSocket webhook (`//ws`).
...
7. **PlaceholderConfig vs implementation**: `PlaceholderConfig` appears in 6 channel configs (Telegram, Discord, Slack, LINE, OneBot, ), but only channels that implement both `PlaceholderCapable` + `MessageEditor` (Telegram, Discord, ) can actually use placeholder message editing. The rest are reserved fields.

8. **ReasoningChannelID**: Most channel configs include a `reasoning_channel_id` field to route LLM reasoning/thinking output to a designated channel (WhatsApp, Telegram, Feishu, Discord, MaixCam, QQ, DingTalk, Slack, LINE, OneBot, WeCom, WeComApp). Note: `Config` does not currently expose this field. `BaseChannel` exposes this via the `WithReasoningChannelID` option and `ReasoningChannelID()` method.