...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in Clotho.
...
10. [Creating a New Provider in Clotho](#creating-a-new-provider-in-clotho)
...
// IMPORTANT: These are EXAMPLE values only - replace with your own OAuth credentials
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<ClothoConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- Clotho codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.clotho/auth.json`:
...
## Creating a New Provider in Clotho

Clotho providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/clotho/cmd_auth.go`:
...
clotho auth login --provider your-provider
...
clotho agent -m "Hello" --model your-model
```
...
export CLOTHO_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export CLOTHO_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `clotho auth login --provider antigravity`
...
- Check auth profile storage: `~/.clotho/auth.json`
- Re-run `clotho auth login --provider antigravity`
