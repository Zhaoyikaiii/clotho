...
Claw's tools configuration is located in the `tools` field of `config.json`.
...
By default, Claw blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `CLOTHO_TOOLS_<SECTION>_<KEY>`:

For example:

- `CLOTHO_TOOLS_WEB_BRAVE_ENABLED=true`
- `CLOTHO_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `CLOTHO_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
- `CLOTHO_TOOLS_MCP_ENABLED=true`
