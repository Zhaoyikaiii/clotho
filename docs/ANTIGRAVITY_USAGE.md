# Using Antigravity Provider in Clotho

This guide explains how to set up and use the **Antigravity** (Google Cloud Code Assist) provider in Clotho.
...
```bash
clotho auth login --provider antigravity
```
...
6.  **Paste it back into the terminal** where Clotho is waiting.

Clotho will extract the authorization code and complete the process automatically.
...
### Switch Models
You can change the default model in `~/.clotho/config.json` or override it via the CLI:

```bash
# Override for a single command
clotho agent -m "Hello" --model claude-opus-4-6-thinking
```
...
1.  **Environment Variables**:
    *   `CLOTHO_AGENTS_DEFAULTS_MODEL=gemini-flash`
2.  **Authentication persistence**: 
    If you've logged in locally, you can copy your credentials to the server:
    ```bash
    scp ~/.clotho/auth.json user@your-server:~/.clotho/
    ```
...
*   **429 Rate Limit**: Antigravity has strict quotas. Clotho will display the "reset time" in the error message if you hit a limit.
*   **404 Not Found**: Ensure you are using a model ID from the `clotho auth models` list. Use the short ID (e.g., `gemini-3-flash`) not the full path.
